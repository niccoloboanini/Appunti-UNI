...
Scegliamo $\boxed{\text{TTRT}=12}$ (cfr. dopo perché questa scelta)

2) Costruisco le colonne di $\text{TRT}$ e $\text{THT}$
![[Pasted image 20220718201945.png|500]]
> Lettura tabella: il token al primo nodo ha tempo di rotazione $0$. Si confronta quindi $0$ con $12$. Essendo $12-0\geq 0$, si carica il contatore $\text{THT}$ con il valore $12$. Di questi, due unità sono impiegate per l'accesso $\alpha_{i}$, quindi rimane un *residuo* di $10$, destinato a un traffico asincrono.
> Il token arriva quindi alla stazione $2$ 